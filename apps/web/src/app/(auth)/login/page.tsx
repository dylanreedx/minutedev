...
export default function LoginPage() {
  ...
  const handlePasswordSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        if (error.status === 403) {
          // Email not verified - send OTP and show verification step
          toast.info("Please verify your email. Sending verification code...");
          try {
            ...
            if (!response.ok || data.error) {
              toast.error(data.message || data.error?.message || "Failed to send verification code");
              return;
            }
  ...
  const handleVerifyEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // After verification, try signing in again
